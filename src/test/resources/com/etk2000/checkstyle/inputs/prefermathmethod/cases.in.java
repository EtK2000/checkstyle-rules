...
class InputPreferMathMethodTernaryAbsWithFieldAccessSliceViolation {
	...
	int m(InputPreferMathMethodTernaryAbsWithFieldAccessSliceViolation a) {
		return a.x < 0 ? -a.x : a.x; // violation: Use 'Math.abs(a.x)' instead of 'a.x < 0 ? -a.x : a.x'.
	}
}
// === end ===
...
class InputPreferMathMethodClampCastArgSliceViolation {
	int m(int value, long lo, int hi) {
		return Math.max((int) lo, Math.min(hi, value)); // violation [minSdk>=35]: Use 'Math.clamp(value, (int) lo, hi)' instead of 'Math.max((int) lo, Math.min(hi, value))'.
	}
}
// === end ===
...
class InputPreferMathMethodClampCommentParenInArgSliceViolation {
	int m(int lo, int value, int hi) {
		return Math.max(lo /* ) */, Math.min(hi, value)); // violation [minSdk>=35]: Use 'Math.clamp(value, lo, hi)' instead of 'Math.max(lo, Math.min(hi, value))'.
	}
}
// === end ===
...
class InputPreferMathMethodClampLiteralParenInArgSliceViolation {
	int m(String s, int value, int hi) {
		return Math.max(s.indexOf(')'), Math.min(hi, value)); // violation [minSdk>=35]: Use 'Math.clamp(value, s.indexOf(')'), hi)' instead of 'Math.max(s.indexOf(')'), Math.min(hi, value))'.
	}
}
// === end ===
...
class InputPreferMathMethodClampMaxMinNestedInnerArgSliceViolation {
	int m(int lo, int hi, int a, int b) {
		return Math.max(lo, Math.min(hi, foo(a, b))); // violation [minSdk>=35]: Use 'Math.clamp(foo(a, b), lo, hi)' instead of 'Math.max(lo, Math.min(hi, foo(a, b)))'.
	}
}
// === end ===
...
class InputPreferMathMethodClampMaxMinNestedOuterArgSliceViolation {
	int m(int x, int y, int hi, int value) {
		return Math.max(bar(x, y), Math.min(hi, value)); // violation [minSdk>=35]: Use 'Math.clamp(value, bar(x, y), hi)' instead of 'Math.max(bar(x, y), Math.min(hi, value))'.
	}
}
// === end ===
...
class InputPreferMathMethodClampMaxMinReversedArgsSliceViolation {
	int m(int value, int lo, int hi) {
		return Math.max(Math.min(hi, value), lo); // violation [minSdk>=35]: Use 'Math.clamp(value, lo, hi)' instead of 'Math.max(Math.min(hi, value), lo)'.
	}
}
// === end ===
...
class InputPreferMathMethodClampMaxMinReversedNestedArgSliceViolation {
	int m(int hi, int a, int b, int lo) {
		return Math.max(Math.min(hi, foo(a, b)), lo); // violation [minSdk>=35]: Use 'Math.clamp(foo(a, b), lo, hi)' instead of 'Math.max(Math.min(hi, foo(a, b)), lo)'.
	}
}
// === end ===
...
class InputPreferMathMethodClampMinMaxSliceViolation {
	int m(int value, int lo, int hi) {
		return Math.min(hi, Math.max(lo, value)); // violation [minSdk>=35]: Use 'Math.clamp(value, lo, hi)' instead of 'Math.min(hi, Math.max(lo, value))'.
	}
}
// === end ===
...
class InputPreferMathMethodClampMinMaxNestedInnerArgSliceViolation {
	int m(int hi, int lo, int a, int b) {
		return Math.min(hi, Math.max(lo, foo(a, b))); // violation [minSdk>=35]: Use 'Math.clamp(foo(a, b), lo, hi)' instead of 'Math.min(hi, Math.max(lo, foo(a, b)))'.
	}
}
// === end ===
...
class InputPreferMathMethodClampMinMaxReversedArgsSliceViolation {
	int m(int value, int lo, int hi) {
		return Math.min(Math.max(lo, value), hi); // violation [minSdk>=35]: Use 'Math.clamp(value, lo, hi)' instead of 'Math.min(Math.max(lo, value), hi)'.
	}
}
// === end ===
...
class InputPreferMathMethodClampMinMaxReversedNestedArgSliceViolation {
	int m(int lo, int a, int b, int hi) {
		return Math.min(Math.max(lo, foo(a, b)), hi); // violation [minSdk>=35]: Use 'Math.clamp(foo(a, b), lo, hi)' instead of 'Math.min(Math.max(lo, foo(a, b)), hi)'.
	}
}
// === end ===
...
class InputPreferMathMethodClampStringParenInArgSliceViolation {
	int m(int value, int hi) {
		return Math.max("(".length(), Math.min(hi, value)); // violation [minSdk>=35]: Use 'Math.clamp(value, "(".length(), hi)' instead of 'Math.max("(".length(), Math.min(hi, value))'.
	}
}
// === end ===
...
class InputPreferMathMethodClampTernaryArgSliceViolation {
	int m(int value, int a, int b, int hi, boolean cond) {
		return Math.max(cond ? a : b, Math.min(hi, value)); // violation [minSdk>=35]: Use 'Math.clamp(value, cond ? a : b, hi)' instead of 'Math.max(cond ? a : b, Math.min(hi, value))'.
	}
}
// === end ===
...
class InputPreferMathMethodIfAbsDeclArrayTargetSliceViolation {
	int m(int[] r, int i, int a) {
		if (a < 0) // violation: Replace this if-else with 'Math.abs(a)'.
			r[i] = -a;
		else
			r[i] = a;
		return r[i];
...
class InputPreferMathMethodIfAbsDeclGeZeroLeftSliceViolation {
	int m(int a) {
		int r;
		if (0 >= a) // violation: Replace this if-else with 'Math.abs(a)'.
			r = -a;
		else
			r = a;
		return r;
...
class InputPreferMathMethodIfComparisonNotEqualitySliceViolation {
	int m(int a, int b) {
		final int r;
		if (a < b) // violation: Replace this if-else with 'Math.min(a, b)'.
			r = a;
		else
			r = b;
		return r;
...
class InputPreferMathMethodIfInitOverwriteMergeIntoDeclSliceViolation {
	void m(int a, int b) {
		var r = b;
		if (a > b) // violation: Replace this if-else with 'Math.max(a, b)'.
			r = a;
...
class InputPreferMathMethodIfMaxElseIfChainSliceViolation {
	int m(boolean cond, int a, int b) {
		if (cond)
			return 0;
		else if (a > b) // violation: Replace this if-else with 'Math.max(a, b)'.
			return a;
		else
			return b;
	}
