...
| PreferSpecificApiCheck               | Prefer `.getFirst()` over `.get(0)`, `.getLast()` over `.get(size()-1)`, `.isEmpty()` over `.size() == 0`, `.toList()` over `.collect(Collectors.toList())`. Uses reflection to verify the receiver type has the suggested method                                                                                                                                                                                  |
