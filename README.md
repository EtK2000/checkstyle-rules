...
| PreferCollectionInterfaceCheck | Method and constructor signatures must use collection interface types (List, Set, Map) instead of concrete implementations (ArrayList, HashSet, HashMap, etc.). Checks return types, parameter types, and record components. A type nested inside another type's type arguments is left alone, since generics are invariant and `Map<String, List<X>>` is not a supertype of `Map<String, ArrayList<X>>`; the exception is a `? extends` bound on a parameter, which genuinely widens. A signature is also left alone when replacing its concrete collections would make it identical to another overload's (`dump(List<String>)` alongside `dump(ArrayList<String>)`), since that would stop compiling, or identical to an overload inherited from a same-file supertype or subtype, since that would silently turn the overload pair into an override; when the body uses a parameter somewhere that demands the concrete type, decided against the target of each use (a call the interface lacks, an assignment into a concrete-typed variable, an argument or return the other side would reject); and when the file already binds the replacement simple name, whether by declaring it or importing it from elsewhere. Everything else is reported, but only auto-fixed when the rewrite cannot escape the file: a parameter when nothing outside can override the member (a constructor, a private/static/final member, or one whose declaring type is an enum, record, annotation, final class, or a type nothing can name to extend), and a return type when nothing outside can name the member to call it (private, or declared in an enum constant body, anonymous class, or local class). Anything else is a warning, which the fix task skips |
...
| [Syntax permutation catalogue](docs/syntax-permutation-catalogue.md) | Java syntax variants that share AST skeletons and are most often under-tested |
