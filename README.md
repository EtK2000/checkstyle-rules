...
| PreferVarCheck                       | For-each loops, try-with-resources, and local variables must use `var`. Auto-detects generic return types via reflection and same-class AST analysis; configurable `allowedMethods` fallback (e.g. `findViewById`) |
